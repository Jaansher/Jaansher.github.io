...
const setData = () => {
    ...
    firebase.database().ref("Teams/" + firebase.auth().currentUser.uid).once("child_added", (data) => {
        ...
        if (teamsData.admin === firebase.auth().currentUser.email) {
            ...
            const ques = document.getElementById("questions").getElementsByTagName("input");
            if(teamsData.Questions){
                ques[0].value = teamsData.Questions[0];
                ques[1].value = teamsData.Questions[1];
                ques[2].value = teamsData.Questions[2];
            }
...
document.getElementById("report-Date").append(new Date().getDate() + " " + months[new Date().getMonth()] + " " + new Date().getFullYear());
